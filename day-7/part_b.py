...
@lru_cache(maxsize=None)
def get_eventual_cost(name: str) -> int:
    cost = 1
    for k, v in bag_dict[name]:
        cost += k * get_eventual_cost(v)
    return cost
...
# must exclude the outermost shiny gold bag...
# IDIOT
print(get_eventual_cost('shiny gold') - 1)
