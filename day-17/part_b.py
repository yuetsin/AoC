...
def get_neighbors(x: int, y: int, z: int, w: int) -> list[tuple]:
    result = []
    for rx in x - 1, x, x + 1:
        for ry in y - 1, y, y + 1:
            for rz in z - 1, z, z + 1:
                for rw in w - 1, w, w + 1:
                    result.append((rx, ry, rz, rw))
...
def get_w_range(grids) -> tuple[int]:
    w_pos = [pos[3] for pos in grids]
    return range(min(w_pos) - 1, max(w_pos) + 2)
    ...
    for x in get_x_range(area):
        for y in get_y_range(area):
            for z in get_z_range(area):
                for w in get_w_range(area):
                    light_neighbors = sum([1 if ng in area else 0 for ng in get_neighbors(x, y, z, w)])
                    if (x, y, z, w) in area and light_neighbors == 2 or light_neighbors == 3:
                        new_area.add((x, y, z, w))
                    elif not (x, y, z, w) in area and light_neighbors == 3:
                        new_area.add((x, y, z, w))   
    area = new_area
    cycle_count += 1
